...
public:
    ...
    BPTree(std::string fileName, int sizeofKey, int degree);
...
template<typename T>
BPTree<T>::BPTree(std::string fileName, int sizeofKey, int degree) : 
    fileName(fileName), sizeofKey(sizeofKey), degree(degree), 
    keyCount(0), nodeCount(0), level(0), root(nullptr), head(nullptr) 
{
    initBPTree();
}
