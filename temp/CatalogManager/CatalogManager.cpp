	...
	if (outfile.is_open())
	{
		for (int tN = 0; tN < getTableNum(); tN++)
		{
			...
			for (int aN = 0; aN < tables[tN].attributeNum; aN++)
			{
				...
				if (tables[tN].attributes[aN].type == INT)
				{
					outfile << "int";
				...
				if (tables[tN].attributes[aN].type == CHAR)
				{
					outfile << "char" << " ";
